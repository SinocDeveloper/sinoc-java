...
import org.sinoc.net.submit.TransactionExecutor;
import org.sinoc.net.submit.TransactionTask;
...
import static org.sinoc.net.message.ReasonCode.USELESS_PEER;
import static org.sinoc.sync.PeerState.*;
import static org.sinoc.util.ByteUtil.toHexString;
import static org.sinoc.util.Utils.longToTimePeriod;
...
public class Eth62 extends EthHandler {
    ...
    protected synchronized void processNewBlock(NewBlockMessage newBlockMessage) {
        ...
        BigInteger minDeadLine = config.getMinDeadLine();
        int maxWaitsecond = config.getMaxWaitDeadLine();
        long now = System.currentTimeMillis();
        if(newBlock.getTimestamp()*1000 - now > 30 * 1000){
        	logger.info("Peer has a error system clock,now:{},block timestamp:{}",now,newBlock.getTimestamp()*1000);
        	dropConnection();
        	return;
        }
        if(minDeadLine.compareTo(newBlock.getDeadLineBI()) < 0){
        	if(now-newBlock.getTimestamp()*1000<(maxWaitsecond-30)*1000){
        		logger.info("Peer is a Attacker, it don't wait for other peer scan disk,now:{},block timestamp:{}",now,newBlock.getTimestamp()*1000);
        		dropConnection();
        		return;
        	}
        }
